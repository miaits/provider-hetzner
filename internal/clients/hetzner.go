...
const (
	...
	errUnmarshalCredentials = "cannot unmarshal hetzner credentials as JSON"
)

const (
	keyToken           = "token"
	keyEndpoint        = "endpoint"
	keyEndpointHetzner = "endpoint_hetzner"
	keyPollInterval    = "poll_interval"
	keyPollFunction    = "poll_function"
)
...
func TerraformSetupBuilder(version, providerSource, providerVersion string) terraform.SetupFn {
	return func(ctx context.Context, client client.Client, mg resource.Managed) (terraform.Setup, error) {
		...
		if v, ok := creds[keyEndpointHetzner]; ok {
			ps.Configuration[keyEndpointHetzner] = v
		}
		if v, ok := creds[keyPollInterval]; ok {
			ps.Configuration[keyPollInterval] = v
		}
		if v, ok := creds[keyPollFunction]; ok {
			ps.Configuration[keyPollFunction] = v
		}
		return ps, nil
	}
}
